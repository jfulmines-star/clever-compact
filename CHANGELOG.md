## v2.3.1 — May 18, 2026
**OC 2026.5.2 compatibility fix — `before_prompt_build` injection restored for all agents**
- Fixed: `hasInjectedThisSession` was declared at module level in `index.ts` (v2.2.3). OC 2026.5.2+ calls `register(api)` once per agent session, so the module-level flag persisted across sessions — every session after the first silently skipped injection.
- Fix: flag moved inside `register()` function scope, matching the already-correct installed `index.js` (v2.2.4).
- `package.json` `openclaw.extensions` corrected to `["./index.js"]` (was `["./index.ts"]` — OC 2026.4.10+ cannot execute TypeScript natively).
- All version references unified to `2.3.1` across `index.js`, `index.ts`, `package.json`, `openclaw.plugin.json`, `SKILL.md`.
- No functional changes to state file format, injection behavior, or write-side API — existing compact-state files carry over automatically.
- No gateway restart required; fix takes effect on next session start after `openclaw plugins registry --refresh`.
